...
class MonnifyPaymentService:
    ...
    def get_access_token():
        """
        Get access token from Monnify auth endpoint using Basic Auth
        Monnify requires first step of authentication: /api/v1/auth/login with API_KEY:SECRET_KEY
        
        Returns:
            str: Access token for Bearer authentication
        """
        # Use cached token if still valid
        if MonnifyPaymentService._access_token and MonnifyPaymentService._token_expiry:
            if datetime.utcnow() < MonnifyPaymentService._token_expiry:
                print(f'[AUTH DEBUG] Using cached access token')
                return MonnifyPaymentService._access_token
        
        # Need to authenticate to get new token
        auth_url = f'{MonnifyPaymentService.BASE_URL}/api/v1/auth/login'
        
        # Basic Auth: base64 encode API_KEY:SECRET_KEY
        ...
        print(f'[AUTH DEBUG] API_KEY: {MonnifyPaymentService.API_KEY}')
        print(f'[AUTH DEBUG] SECRET_KEY: {MonnifyPaymentService.SECRET_KEY}')
        print(f'[AUTH DEBUG] Credentials format: {credentials}')
        print(f'[AUTH DEBUG] Base64 encoded: {encoded}')
        print(f'[AUTH DEBUG] Full auth header: {headers["Authorization"]}')
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f'[AUTH DEBUG] Auth Response: {json.dumps(data, indent=2)}')
                
                if data.get('requestSuccessful'):
                    access_token = data.get('responseBody', {}).get('accessToken')
                    if access_token:
                        # Cache token for future use (assume 1 hour validity)
                        MonnifyPaymentService._access_token = access_token
                        MonnifyPaymentService._token_expiry = datetime.utcnow() + timedelta(hours=1)
                        
                        print(f'[AUTH DEBUG] Access token obtained: {access_token[:50]}...')
                        return access_token
                    else:
                        raise ValueError('No access token in response')
                else:
                    error_msg = data.get('responseMessage', 'Authentication failed')
                    print(f'[AUTH ERROR] {error_msg}')
                    raise ValueError(f'Authentication failed: {error_msg}')
            else:
                print(f'[AUTH ERROR] Auth endpoint returned {response.status_code}: {response.text}')
                raise ValueError(f'Auth failed with status {response.status_code}')
                
        except requests.RequestException as e:
            print(f'[AUTH ERROR] Request exception: {str(e)}')
            raise ValueError(f'Auth request failed: {str(e)}')
    
    @staticmethod
    def get_auth_header():
        """
        Generate Auth header with Bearer token for API requests
        First obtains access token from login endpoint, then uses it for Bearer auth
        
        Returns:
            dict: Authorization header with access token
        """
        access_token = MonnifyPaymentService.get_access_token()
    ...
    def initiate_payment(user_id, amount_naira):
        ...
        try:
            ...
            api_url = f'{MonnifyPaymentService.BASE_URL}/api/v1/transactions/init'
            print(f'[MONNIFY API] Calling: {api_url}')
            print(f'[MONNIFY API] Headers: {headers}')
            print(f'[MONNIFY API] Full Payload: {payload}')
            print(f'[MONNIFY API] Payload JSON: {json.dumps(payload, indent=2)}')
            ...
            print(f'[MONNIFY API] Response Status: {response.status_code}')
            print(f'[MONNIFY API] Response Headers: {dict(response.headers)}')
            print(f'[MONNIFY API] Response Body: {response.text}')
            ...
            if response.status_code in [200, 201]:
                ...
                print(f'[ERROR] Monnify API returned {response.status_code}')
                print(f'[ERROR] Response: {response.text}')
                print(f'[ERROR] Parsed error data: {error_data}')
                return {
                    'success': False,
                    'error': error_data.get('responseMessage', f'Payment initialization failed (HTTP {response.status_code})'),
    ...
    def verify_payment(reference):
        ...
        try:
            ...
            response = requests.get(
                f'{MonnifyPaymentService.BASE_URL}/api/v1/transactions/query?transactionReference={reference}',
                headers=headers,
                timeout=30
            )
